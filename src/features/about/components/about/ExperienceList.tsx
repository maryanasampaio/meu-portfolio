      ...
      {experiences.map((exp) => (
        ...
        >
          ...
          <header className="z-10 mb-2 mt-1 sm:col-span-2">
            <div className="text-sm font-semibold text-slate-200 mb-1">{exp.company}</div>
            ...
            {exp.technologies.length > 0 && (
              <div className="mt-3 flex flex-wrap" aria-label="Tecnologias usadas">
                {exp.technologies.map((tech) => (
                  <Badge
                    key={tech}
                    className="mr-1.5 mt-2 rounded-full bg-teal-400/10 px-3 py-1 text-xs font-medium leading-5 text-teal-300 border-transparent"
                    variant="outline"
                  >
                    {tech}
                  </Badge>
